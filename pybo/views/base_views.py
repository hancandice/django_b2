...
def index(request):
    ...
    if so == "recommend":
        questionList = Question.objects.annotate(numVoter=Count('voter')).order_by('-numVoter', '-createDate')
    elif so == "popular":
        questionList = Question.objects.annotate(numAnswer=Count('answer')).order_by('-numAnswer', '-createDate')
    else:
